...
void CartesianImpedanceExampleController::publishDebug(const ros::Time& time) {
  if (publisher_franka_debug_.trylock()) {
      for (size_t i = 0; i < 3; i++) {
        publisher_franka_debug_.msg_.position_d[i] = position_d_[i];
        publisher_franka_debug_.msg_.position_d_target[i] = position_d_target_[i];
        publisher_franka_debug_.msg_.position_error[i] = error[i];
        publisher_franka_debug_.msg_.position[i] = position[i];
      }
      publisher_franka_debug_.unlockAndPublish();
    }
}
